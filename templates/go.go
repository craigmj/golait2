...
func HttpHandlerFunc(w http.ResponseWriter, r *http.Request) {
	{{if $ConnectionClass -}}
		...
		conn, err := _root.{{$ConnectionClassConstructor}}(w,r)
		if nil!=err {
			Log.Errorf("Error creating ConnectionClass {{.ConnectionClass}}: %s", err.Error())
			http.Error(w,err.Error(), http.StatusInternalServerError)
			return
		}
		{{else}}
		conn := &_root.{{$ConnectionClass}}{}
		{{end}}	
	{{else}}
	...
	{{if $ConnectionClass}}
	ProcessJsonRpc(r.Body, w, conn)
	{{else}}
	ProcessJsonRpc(r.Body, w, context)
	{{end}}
...
{{if $ConnectionClass}}
func ProcessJsonRpc(in io.Reader, out io.Writer, conn *_root.{{$ConnectionClass}}) {
{{else}}
func ProcessJsonRpc(in io.Reader, out io.Writer, context *_root.{{.ClassName}}) {
{{end}}
		...
		*/}}
		...
		if len(request.Params)!={{.ParametersLen}} {
			errorJsonRpc(out, request.Id, JSONRPC_ERROR_INVALID_PARAMS, fmt.Errorf(
				"Expected %d parameters, but got %d in call to %s", {{.ParametersLen}}, len(request.Params), "{{.Name}}"), nil)
			return
		}
		{{range $i, $p := .Parameters}}
